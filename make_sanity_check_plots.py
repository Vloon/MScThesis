...
arguments = [('-df', 'data_folder', str, 'Data'), # data folder
             ('-bdf', 'base_data_filename', str, 'processed_data'),  # the most basic version of the filename of the saved data
             ...
             # ('-hcolor', 'hist_color', str, 'k'), # histogram color
...
for si, n_sub in enumerate(range(subject1, subjectn + 1)):
    for ti, task in enumerate(tasks):
        ...
        distance_trace = get_attribute_from_trace(embedding.particles['_z'], det_params_func, 'd_norm') # n_particles x M
        sigma_div_bound_trace = invlogit(embedding.particles['sigma_beta_T']) # n_particles x 1
        bound_trace = get_attribute_from_trace(embedding.particles['_z'], det_params_func, 'bound') # n_particles x M
        ...
        if ground_truth:
            ...
            gt_distances = det_params_func(gt_embedding['_z'])['d_norm']
            gt_bound = det_params_func(gt_embedding['_z'])['bound']
